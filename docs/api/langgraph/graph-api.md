...
{' ' * (self.list_depth - 1)}- 
[`State`](#state): A shared data structure that represents the current snapshot of your application. It can be any data type, but is typically defined using a shared state schema.

{' ' * (self.list_depth - 1)}- 
[`Nodes`](#nodes): Functions that encode the logic of your agents. They receive the current state as input, perform some computation or side-effect, and return an updated state.

{' ' * (self.list_depth - 1)}- 
[`Edges`](#edges): Functions that determine which `Node` to execute next based on the current state. They can be conditional branches or fixed transitions.
...
{' ' * (self.list_depth - 1)}- Internal nodes can pass information that is not required in the graph’s input / output.

{' ' * (self.list_depth - 1)}- We may also want to use different input / output schemas for the graph. The output might, for example, only contain a single relevant output key.
...
{' ' * (self.list_depth - 1)}- `state`—The [state](#state) of the graph

{' ' * (self.list_depth - 1)}- `config`—A [`RunnableConfig`](https://reference.langchain.com/python/langchain-core/runnables/config/RunnableConfig) object that contains configuration information like `thread_id` and tracing information like `tags`

{' ' * (self.list_depth - 1)}- `runtime`—A `Runtime` object that contains [runtime `context`](#runtime-context) and other information like `store`, `stream_writer`, `execution_info`, `server_info`, `heartbeat` (for idle timeout refresh), and `control` (for [graceful shutdown](/oss/python/langgraph/durable-execution#graceful-shutdown))
...
{' ' * (self.list_depth - 1)}- Specify a cache when compiling a graph (or specifying an entrypoint)

{' ' * (self.list_depth - 1)}- Specify a cache policy for nodes. Each cache policy supports:

{' ' * (self.list_depth - 1)}- `key_func` used to generate a cache key based on the input to a node, which defaults to a `hash` of the input with pickle.

{' ' * (self.list_depth - 1)}- `ttl`, the time to live for the cache in seconds. If not specified, the cache will never expire.
...
{' ' * (self.list_depth - 1)}- First run takes two seconds to run (due to mocked expensive computation).

{' ' * (self.list_depth - 1)}- Second run utilizes cache and returns quickly.

## [​](#edges)Edges

Edges define how the logic is routed and how the graph decides to stop. This is a big part of how your agents work and how different nodes communicate with each other. There are a few key types of edges:

{' ' * (self.list_depth - 1)}- Normal Edges: Go directly from one node to the next.

{' ' * (self.list_depth - 1)}- Conditional Edges: Call a function to determine which node(s) to go to next.

{' ' * (self.list_depth - 1)}- Entry Point: Which node to call first when user input arrives.

{' ' * (self.list_depth - 1)}- Conditional Entry Point: Call a function to determine which node(s) to call first when user input arrives.
...
{' ' * (self.list_depth - 1)}- `update`: Apply state updates (similar to returning updates from a node).

{' ' * (self.list_depth - 1)}- `goto`: Navigate to specific nodes (similar to [conditional edges](#conditional-edges)).

{' ' * (self.list_depth - 1)}- `graph`: Target a parent graph when navigating from [subgraphs](/oss/python/langgraph/use-subgraphs).

{' ' * (self.list_depth - 1)}- `resume`: Provide a value to resume execution after an [interrupt](/oss/python/langgraph/interrupts).

`Command` is used in three contexts:

{' ' * (self.list_depth - 1)}- **[Return from nodes](#return-from-nodes)**: Use `update`, `goto`, and `graph` to combine state updates with control flow.

{' ' * (self.list_depth - 1)}- **[Input to `invoke` or `stream`](#input-to-invoke-or-stream)**: Use `resume` to continue execution after an interrupt.

{' ' * (self.list_depth - 1)}- **[Return from tools](#return-from-tools)**: Similar to return from nodes, combine state updates and control flow from inside a tool.
...
### [​](#input-to-invoke-or-stream)Input to `invoke` or `stream`
...
{' ' * (self.list_depth - 1)}- For threads at the end of the graph (i.e. not interrupted) you can change the entire topology of the graph (i.e. all nodes and edges, remove, add, rename, etc)

{' ' * (self.list_depth - 1)}- For threads currently interrupted, we support all topology changes other than renaming / removing nodes (as that thread could now be about to enter a node that no longer exists) — if this is a blocker please reach out and we can prioritize a solution.

{' ' * (self.list_depth - 1)}- For modifying state, we have full backwards and forwards compatibility for adding and removing keys

{' ' * (self.list_depth - 1)}- State keys that are renamed lose their saved state in existing threads

{' ' * (self.list_depth - 1)}- State keys whose types change in incompatible ways could currently cause issues in threads with state from before the change — if this is a blocker please reach out and we can prioritize a solution.
...
{' ' * (self.list_depth - 1)}- Graceful degradation within the graph

{' ' * (self.list_depth - 1)}- Can save intermediate state in checkpoints

{' ' * (self.list_depth - 1)}- Better user experience with partial results

{' ' * (self.list_depth - 1)}- Graph completes normally (no exception)

**Reactive advantages:**

{' ' * (self.list_depth - 1)}- Simpler implementation

{' ' * (self.list_depth - 1)}- No need to modify graph logic

{' ' * (self.list_depth - 1)}- Centralized error handling
...
{' ' * (self.list_depth - 1)}- [How to use the Graph API](/oss/python/langgraph/use-graph-api)

{' ' * (self.list_depth - 1)}- [Functional API conceptual overview](/oss/python/langgraph/functional-api)
...
Was this page helpful?YesNo[Choosing between Graph and Functional APIsPrevious](/oss/python/langgraph/choosing-apis)[Use the graph APINext](/oss/python/langgraph/use-graph-api)⌘I[Docs by LangChain home page
