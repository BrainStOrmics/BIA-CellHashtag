...
Learn how to think about building agents with LangGraphCopy page
> 
...
Use this file to discover all available pages before exploring further.When you build an agent with LangGraph, you will first break it apart into discrete steps called **nodes**. Then, you will describe the different decisions and transitions from each of your nodes. Finally, you connect nodes together through a shared **state** that each node can read from and write to.
...
{' ' * (self.list_depth - 1)}- `Read Email`: Extract and parse the email content

{' ' * (self.list_depth - 1)}- `Classify Intent`: Use an LLM to categorize urgency and topic, then route to appropriate action

{' ' * (self.list_depth - 1)}- `Doc Search`: Query your knowledge base for relevant information

{' ' * (self.list_depth - 1)}- `Bug Track`: Create or update issue in tracking system

{' ' * (self.list_depth - 1)}- `Draft Reply`: Generate an appropriate response

{' ' * (self.list_depth - 1)}- `Human Review`: Escalate to human agent for approval or handling

{' ' * (self.list_depth - 1)}- `Send Reply`: Dispatch the email response
...
{' ' * (self.list_depth - 1)}- Static context (prompt): Classification categories, urgency definitions, response format

{' ' * (self.list_depth - 1)}- Dynamic context (from state): Email content, sender information

{' ' * (self.list_depth - 1)}- Desired outcome: Structured classification that determines routing

Draft reply

{' ' * (self.list_depth - 1)}- Static context (prompt): Tone guidelines, company policies, response templates

{' ' * (self.list_depth - 1)}- Dynamic context (from state): Classification results, search results, customer history

{' ' * (self.list_depth - 1)}- Desired outcome: Professional email response ready for review
...
{' ' * (self.list_depth - 1)}- Parameters: Query built from intent and topic

{' ' * (self.list_depth - 1)}- Retry strategy: Yes, with exponential backoff for transient failures

{' ' * (self.list_depth - 1)}- Caching: Could cache common queries to reduce API calls

Customer history lookup

{' ' * (self.list_depth - 1)}- Parameters: Customer email or ID from state

{' ' * (self.list_depth - 1)}- Retry strategy: Yes, but with fallback to basic info if unavailable

{' ' * (self.list_depth - 1)}- Caching: Yes, with time-to-live to balance freshness and performance
...
{' ' * (self.list_depth - 1)}- When to execute node: After approval (human or automated)

{' ' * (self.list_depth - 1)}- Retry strategy: Yes, with exponential backoff for network issues

{' ' * (self.list_depth - 1)}- Should not cache: Each send is a unique action

Bug track

{' ' * (self.list_depth - 1)}- When to execute node: Always when intent is “bug”

{' ' * (self.list_depth - 1)}- Retry strategy: Yes, critical to not lose bug reports

{' ' * (self.list_depth - 1)}- Returns: Ticket ID to include in response
...
{' ' * (self.list_depth - 1)}- Context for decision: Original email, draft response, urgency, classification

{' ' * (self.list_depth - 1)}- Expected input format: Approval boolean plus optional edited response

{' ' * (self.list_depth - 1)}- When triggered: High urgency, complex issues, or quality concerns
...
{' ' * (self.list_depth - 1)}- The original email and sender info (can’t reconstruct these later)

{' ' * (self.list_depth - 1)}- Classification results (needed by multiple later/downstream nodes)

{' ' * (self.list_depth - 1)}- Search results and customer data (expensive to re-fetch)

{' ' * (self.list_depth - 1)}- The draft response (needs to persist through review)

{' ' * (self.list_depth - 1)}- Execution metadata (for debugging and recovery)
...
{' ' * (self.list_depth - 1)}- Different nodes can format the same data differently for their needs

{' ' * (self.list_depth - 1)}- You can change prompt templates without modifying your state schema

{' ' * (self.list_depth - 1)}- Debugging is clearer—you see exactly what data each node received

{' ' * (self.list_depth - 1)}- Your agent can evolve without breaking existing state
...
{' ' * (self.list_depth - 1)}- Transient errors
{' ' * (self.list_depth - 1)}- LLM-recoverable
{' ' * (self.list_depth - 1)}- User-fixable
{' ' * (self.list_depth - 1)}- Unexpected
{' ' * (self.list_depth - 1)}- Saga / compensationAdd a retry policy to automatically retry network issues and rate limits.Combine with `timeout=` to cap each attempt. See [Fault tolerance](/oss/python/langgraph/fault-tolerance) for the full lifecycle.
```
...
```
After retries are exhausted, run a recovery function that updates state and routes to a compensation branch.See [Fault tolerance](/oss/python/langgraph/fault-tolerance#error-handling) for the full pattern.`error_handler` requires `langgraph>=1.2`.
```
...
Node granularity trade-offsThis section explores the trade-offs in node granularity design. Most applications can skip this and use the patterns shown above.You might wonder: why not combine `Read Email` and `Classify Intent` into one node?Or why separate Doc Search from Draft Reply?The answer involves trade-offs between resilience and observability.**The resilience consideration:** LangGraph’s [durable execution](/oss/python/langgraph/durable-execution) creates checkpoints at node boundaries. When a workflow resumes after an interruption or failure, it starts from the beginning of the node where execution stopped. Smaller nodes mean more frequent checkpoints, which means less work to repeat if something goes wrong. If you combine multiple operations into one large node, a failure near the end means re-executing everything from the start of that node.Why we chose this breakdown for the email agent:
...
[Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.[Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss/langgraph/thinking-in-langgraph.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).

Was this page helpful?YesNo[ChangelogPrevious](/oss/python/langgraph/changelog-py)[Workflows and agentsNext](/oss/python/langgraph/workflows-agents)⌘I[Docs by LangChain home page
