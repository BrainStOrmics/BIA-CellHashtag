...
Use this file to discover all available pages before exploring further.This guide reviews common workflow and agent patterns.

{' ' * (self.list_depth - 1)}- Workflows have predetermined code paths and are designed to operate in a certain order.

{' ' * (self.list_depth - 1)}- Agents are dynamic and define their own processes and tool usage.
...
{' ' * (self.list_depth - 1)}- Install dependencies:
...
{' ' * (self.list_depth - 1)}- Initialize the LLM:
...
{' ' * (self.list_depth - 1)}- Translating documents into different languages

{' ' * (self.list_depth - 1)}- Verifying generated content for consistency
...
{' ' * (self.list_depth - 1)}- Split up subtasks and run them in parallel, which increases speed

{' ' * (self.list_depth - 1)}- Run tasks multiple times to check for different outputs, which increases confidence

Some examples include:

{' ' * (self.list_depth - 1)}- Running one subtask that processes a document for keywords, and a second subtask to check for formatting errors

{' ' * (self.list_depth - 1)}- Running a task multiple times that scores a document for accuracy based on different criteria, like the number of citations, the number of sources used, and the quality of the sources
...
{' ' * (self.list_depth - 1)}- Breaks down tasks into subtasks

{' ' * (self.list_depth - 1)}- Delegates subtasks to workers

{' ' * (self.list_depth - 1)}- Synthesizes worker outputs into a final result
...
Was this page helpful?YesNo[Thinking in LangGraphPrevious](/oss/python/langgraph/thinking-in-langgraph)[PersistenceNext](/oss/python/langgraph/persistence)⌘I[Docs by LangChain home page
