...
Extended example: Compose an essay with an LLMThis example demonstrates how to use the `@task` and `@entrypoint` decorators
...
Extended example: parallel LLM callsThis example demonstrates how to run multiple LLM calls in parallel using `@task`. Each call generates a paragraph on a different topic, and results are joined into a single text output.
```
import uuid
from langchain.chat_models import init_chat_model
from langgraph.func import entrypoint, task
...
{' ' * (self.list_depth - 1)}- Import [`get_stream_writer`](https://reference.langchain.com/python/langgraph/config/get_stream_writer) from `langgraph.config`.

{' ' * (self.list_depth - 1)}- Obtain a stream writer instance within the entrypoint.

{' ' * (self.list_depth - 1)}- Emit custom data before computation begins.

{' ' * (self.list_depth - 1)}- Emit another custom message after computing the result.

{' ' * (self.list_depth - 1)}- Use `.stream()` to process streamed output.

{' ' * (self.list_depth - 1)}- Specify which streaming modes to use.
...
## [​](#set-task-and-entrypoint-timeouts)Set task and entrypoint timeouts
...
{' ' * (self.list_depth - 1)}- `ttl` is specified in seconds. The cache will be invalidated after this time.
...
{' ' * (self.list_depth - 1)}- Append `"bar"`.

{' ' * (self.list_depth - 1)}- Pause for human input. When resuming, append human input.

{' ' * (self.list_depth - 1)}- Append `"qux"`.
...
{' ' * (self.list_depth - 1)}- Accept the tool call

{' ' * (self.list_depth - 1)}- Revise the tool call and continue

{' ' * (self.list_depth - 1)}- Generate a custom tool message (e.g., instructing the model to re-format its tool call)
...
{' ' * (self.list_depth - 1)}- You want to return a computed result (e.g., a summary or status), but save a different internal value for use on the next invocation.

{' ' * (self.list_depth - 1)}- You need to control what gets passed to the previous parameter on the next run.
...
{' ' * (self.list_depth - 1)}- [Workflows and agent](/oss/python/langgraph/workflows-agents) guide for more examples of how to build workflows using the Functional API.

## [​](#integrate-with-other-libraries)Integrate with other libraries

{' ' * (self.list_depth - 1)}- [Add LangGraph’s features to other frameworks using the functional API](/langsmith/deploy-other-frameworks): Add LangGraph features like persistence, memory and streaming to other agent frameworks that do not provide them out of the box.

---

[Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.[Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss/langgraph/use-functional-api.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).

Was this page helpful?YesNo[Functional API overviewPrevious](/oss/python/langgraph/functional-api)[LangGraph runtimeNext](/oss/python/langgraph/pregel)⌘I[Docs by LangChain home page
