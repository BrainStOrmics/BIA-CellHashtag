...
[LangGraph APIs](/oss/python/langgraph/choosing-apis)[Graph API](/oss/python/langgraph/choosing-apis)
...
{' ' * (self.list_depth - 1)}- How to use state to define a graph’s [schema](/oss/python/langgraph/graph-api#schema)

{' ' * (self.list_depth - 1)}- How to use [reducers](/oss/python/langgraph/graph-api#reducers) to control how state updates are processed.
...
{' ' * (self.list_depth - 1)}- We kicked off invocation by updating a single key of the state.

{' ' * (self.list_depth - 1)}- We receive the entire state in the invocation result.
...
{' ' * (self.list_depth - 1)}- We may wish to update an existing message in the state.

{' ' * (self.list_depth - 1)}- We may want to accept short-hands for [message formats](/oss/python/langgraph/graph-api#using-messages-in-your-graph), such as [OpenAI format](https://python.langchain.com/docs/concepts/messages/#openai-format).
...
### [​](#bypass-reducers-with-overwrite)Bypass reducers with `Overwrite`
...
### [​](#use-pydantic-models-for-graph-state)Use pydantic models for graph state
...
{' ' * (self.list_depth - 1)}- Currently, the output of the graph will **NOT** be an instance of a pydantic model.

{' ' * (self.list_depth - 1)}- Run-time validation only occurs on inputs to the first node in the graph, not on subsequent nodes or outputs.

{' ' * (self.list_depth - 1)}- The validation error trace from pydantic does not show which node the error arises in.

{' ' * (self.list_depth - 1)}- Pydantic’s recursive validation can be slow. For performance-sensitive applications, you may want to consider using a `dataclass` instead.
...
{' ' * (self.list_depth - 1)}- Passing Pydantic objects as inputs

{' ' * (self.list_depth - 1)}- Receiving outputs from the graph

{' ' * (self.list_depth - 1)}- Working with nested Pydantic models
...
Runtime Type CoercionPydantic performs runtime type coercion for certain data types. This can be helpful but also lead to unexpected behavior if you’re not aware of it.
```
from langgraph.graph import StateGraph, START, END
from pydantic import BaseModel
...
Working with Message ModelsWhen working with LangChain message types in your state schema, there are important considerations for serialization. You should use `AnyMessage` (rather than `BaseMessage`) for proper serialization/deserialization when using message objects over the wire.
```
from langgraph.graph import StateGraph, START, END
from pydantic import BaseModel
...
{' ' * (self.list_depth - 1)}- Specify a schema for your configuration

{' ' * (self.list_depth - 1)}- Add the configuration to the function signature for nodes or conditional edges

{' ' * (self.list_depth - 1)}- Pass the configuration into the graph.
...
Extended example: specifying LLM at runtimeBelow we demonstrate a practical example in which we configure what LLM to use at runtime. We will use both OpenAI and Anthropic models.
```
...
Extended example: specifying model and system message at runtimeBelow we demonstrate a practical example in which we configure two parameters: the LLM and system message to use at runtime.
```
from dataclasses import dataclass
from langchain.chat_models import init_chat_model
...
Extended example: customizing retry policiesConsider an example in which we are reading from a SQL database. Below we pass two different retry policies to nodes:
```
import sqlite3
from typing_extensions import TypedDict
from langchain.chat_models import init_chat_model
...
## [​](#set-node-timeouts)Set node timeouts
...
## [​](#configure-node-timeouts)Configure node timeouts
...
## [​](#handle-node-errors)Handle node errors
...
### [​](#access-drain-state-inside-a-node)Access drain state inside a node
...
{' ' * (self.list_depth - 1)}- How to build a sequential graph

{' ' * (self.list_depth - 1)}- Built-in short-hand for constructing similar graphs.
...
{' ' * (self.list_depth - 1)}- How state updates are [checkpointed](/oss/python/langgraph/persistence)

{' ' * (self.list_depth - 1)}- How interruptions are resumed in [human-in-the-loop](/oss/python/langgraph/interrupts) workflows

{' ' * (self.list_depth - 1)}- How we can “rewind” and branch-off executions using LangGraph’s [time travel](/oss/python/langgraph/use-time-travel) features
They also determine how execution steps are [streamed](/oss/python/langgraph/streaming), and how your application is visualized and debugged using [Studio](/langsmith/studio).Let’s demonstrate an end-to-end example. We will create a sequence of three steps:

{' ' * (self.list_depth - 1)}- Populate a value in a key of the state

{' ' * (self.list_depth - 1)}- Update the same value

{' ' * (self.list_depth - 1)}- Populate a different value
Let’s first define our [state](/oss/python/langgraph/graph-api#state). This governs the [schema of the graph](/oss/python/langgraph/graph-api#schema), and can also specify how to apply updates. See [Process state updates with reducers](#process-state-updates-with-reducers) for more detail.In our case, we will just keep track of two values:
```
from typing_extensions import TypedDict
...
```
Note that when issuing updates to the state, each node can just specify the value of the key it wishes to update.By default, this will **overwrite** the value of the corresponding key. You can also use [reducers](/oss/python/langgraph/graph-api#reducers) to control how updates are processed—for example, you can append successive updates to a key instead. See [Process state updates with reducers](#process-state-updates-with-reducers) for more detail.Finally, we define the graph. We use [StateGraph](/oss/python/langgraph/graph-api#stategraph) to define a graph that operates on this state.We will then use [`add_node`](/oss/python/langgraph/graph-api#messagesstate) and [`add_edge`](/oss/python/langgraph/graph-api#edges) to populate our graph and define its control flow.
```
from langgraph.graph import START, StateGraph
...
{' ' * (self.list_depth - 1)}- [`add_edge`](https://reference.langchain.com/python/langgraph/pregel/_draw/add_edge) takes the names of nodes, which for functions defaults to `node.__name__`.

{' ' * (self.list_depth - 1)}- We must specify the entry point of the graph. For this we add an edge with the [START node](/oss/python/langgraph/graph-api#start-node).

{' ' * (self.list_depth - 1)}- The graph halts when there are no more nodes to execute.
...
{' ' * (self.list_depth - 1)}- We kicked off invocation by providing a value for a single state key. We must always provide a value for at least one key.

{' ' * (self.list_depth - 1)}- The value we passed in was overwritten by the first node.

{' ' * (self.list_depth - 1)}- The second node updated the value.

{' ' * (self.list_depth - 1)}- The third node populated a different value.
**Built-in shorthand**
...
{' ' * (self.list_depth - 1)}- You can write regular python code within your node to catch and handle exceptions.

{' ' * (self.list_depth - 1)}- You can set a **[retry_policy](https://langchain-ai.github.io/langgraph/reference/types/#langgraph.types.RetryPolicy)** to direct the graph to retry nodes that raise certain types of exceptions. Only failing branches are retried, so you needn’t worry about performing redundant work.
...
Extended example: loops with branchesTo better understand how the recursion limit works, let’s consider a more complex example. Below we implement a loop, but one step fans out into two nodes:
```
import operator
from typing import Annotated, Literal
from typing_extensions import TypedDict
...
{' ' * (self.list_depth - 1)}- Update `nodes` use `async def` instead of `def`.

{' ' * (self.list_depth - 1)}- Update the code inside to use `await` appropriately.

{' ' * (self.list_depth - 1)}- Invoke the graph with `.ainvoke` or `.astream` as desired.
...
{' ' * (self.list_depth - 1)}- Using Mermaid.ink API (does not require additional packages)

{' ' * (self.list_depth - 1)}- Using Mermaid + Pyppeteer (requires `pip install pyppeteer`)

{' ' * (self.list_depth - 1)}- Using graphviz (which requires `pip install graphviz`)
...
[Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.[Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss/langgraph/use-graph-api.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).

Was this page helpful?YesNo[Graph API overviewPrevious](/oss/python/langgraph/graph-api)[Functional API overviewNext](/oss/python/langgraph/functional-api)⌘I[Docs by LangChain home page
