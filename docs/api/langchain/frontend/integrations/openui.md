...
Generate complete, interactive dashboards and reports using the OpenUI component library and openui-langCopy page
> 
...
{' ' * (self.list_depth - 1)}- **Generate the system prompt:** call `openuiLibrary.prompt()` once at startup; it produces a complete openui-lang reference that the model uses to write valid component trees

{' ' * (self.list_depth - 1)}- **Inject on first message:** send the system prompt as the opening system message when a new conversation starts

{' ' * (self.list_depth - 1)}- **Model writes openui-lang:** the model responds with a program like `root = Stack([header, kpis, chart])` instead of prose

{' ' * (self.list_depth - 1)}- **Render with `Renderer`:** pass the text to OpenUI’s `Renderer` and the component library; it parses and renders the tree
...
## [​](#import-the-component-styles)Import the component styles
...
## [​](#generate-the-system-prompt)Generate the system prompt
...
## [​](#inject-the-system-prompt-via-usestream)Inject the system prompt via useStream
...
## [​](#render-with-the-renderer)Render with the Renderer
...
## [​](#the-openui-lang-format)The openui-lang format
...
## [​](#progressive-rendering-utilities)Progressive rendering utilities
...
{' ' * (self.list_depth - 1)}- **Generate the system prompt at module load:** not inside a React component; the prompt is several kilobytes and should be computed once

{' ' * (self.list_depth - 1)}- **Inject the system prompt only on fresh threads:** check `stream.messages.length === 0` and skip injection on subsequent turns to avoid duplicating the prompt in the thread history

{' ' * (self.list_depth - 1)}- **Use hoisting order:** write `root = Stack([...])` first; the UI shell appears immediately and sections fill in progressively as the model defines each one

{' ' * (self.list_depth - 1)}- **Gate on complete statements:** avoid re-rendering the Renderer on every token; update only when a full statement (`name = ComponentCall(...)`) has arrived

{' ' * (self.list_depth - 1)}- **Verify chart data before rendering:** chart components need their `Series` and label arrays defined before they’re included in the stable snapshot

{' ' * (self.list_depth - 1)}- **Keep camelCase variable names:** the openui-lang parser only accepts camelCase identifiers; reinforce this in the system prompt’s `additionalRules`

---

[Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.[Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss/langchain/frontend/integrations/openui.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).

Was this page helpful?YesNo[assistant-uiPrevious](/oss/python/langchain/frontend/integrations/assistant-ui)[GuardrailsNext](/oss/python/langchain/guardrails)⌘I[Docs by LangChain home page
