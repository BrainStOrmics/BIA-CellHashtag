...
{' ' * (self.list_depth - 1)}- Some providers expect message history to start with a `user` message

{' ' * (self.list_depth - 1)}- Most providers require `assistant` messages with tool calls to be followed by corresponding `tool` result messages.
...
[Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.[Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss/langchain/short-term-memory.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).

Was this page helpful?YesNo[ToolsPrevious](/oss/python/langchain/tools)[Event streamingNext](/oss/python/langchain/event-streaming)⌘I[Docs by LangChain home page
