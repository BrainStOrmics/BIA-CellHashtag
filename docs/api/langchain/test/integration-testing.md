...
Test agents with real LLM APIs by organizing tests, managing keys, handling flakiness, and controlling costs.Copy page
> 
...
## [​](#separate-unit-and-integration-tests)Separate unit and integration tests
...
## [​](#assert-on-structure-not-content)Assert on structure, not content
...
## [​](#reduce-cost-and-latency)Reduce cost and latency

Integration tests that call LLM APIs incur real costs. A few practices help keep test suites fast and affordable:

{' ' * (self.list_depth - 1)}- **Use smaller models**: `gemini-3.1-flash-lite` or equivalent for tests that only need to verify tool calling and response structure.

{' ' * (self.list_depth - 1)}- **Set `maxTokens`**: Cap response length to avoid long, expensive completions.

{' ' * (self.list_depth - 1)}- **Limit test scope**: Test one behavior per test. Avoid end-to-end scenarios that chain many LLM calls when a single-turn test suffices.

{' ' * (self.list_depth - 1)}- **Run selectively**: Use the test separation from [above](#separate-unit-and-integration-tests) to run integration tests only in CI or before deploy, not on every file save.
...
## [​](#record-and-replay-http-calls)Record and replay HTTP calls
...
[Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.[Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss/langchain/test/integration-testing.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).

Was this page helpful?YesNo[Unit testingPrevious](/oss/python/langchain/test/unit-testing)[Agent EvalsNext](/oss/python/langchain/test/evals)⌘I[Docs by LangChain home page
