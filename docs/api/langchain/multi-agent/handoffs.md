...
{' ' * (self.list_depth - 1)}- State-driven behavior: Behavior changes based on a state variable (e.g., `current_step` or `active_agent`)

{' ' * (self.list_depth - 1)}- Tool-based transitions: Tools update the state variable to move between states

{' ' * (self.list_depth - 1)}- Direct user interaction: Each state’s configuration handles user messages directly

{' ' * (self.list_depth - 1)}- Persistent state: State survives across conversation turns
...
Learn how to build a customer support agent using the handoffs pattern, where a single agent transitions between different configurations.Learn more
...
### [​](#single-agent-with-middleware)Single agent with middleware
...
### [​](#multiple-agent-subgraphs)Multiple agent subgraphs
...
Complete example: Sales and support with handoffsThis example shows a multi-agent system with separate sales and support agents. Each agent is a separate graph node, and handoff tools allow agents to transfer conversations to each other.
```
...
{' ' * (self.list_depth - 1)}- **The `AIMessage` containing the tool call** (the message that triggered the handoff)

{' ' * (self.list_depth - 1)}- **A `ToolMessage` acknowledging the handoff** (the artificial response to that tool call)
...
{' ' * (self.list_depth - 1)}- **Context filtering strategy**: Will each agent receive full conversation history, filtered portions, or summaries? Different agents may need different context depending on their role.

{' ' * (self.list_depth - 1)}- **Tool semantics**: Clarify whether handoff tools only update routing state or also perform side effects. For example, should `transfer_to_sales()` also create a support ticket, or should that be a separate action?

{' ' * (self.list_depth - 1)}- **Token efficiency**: Balance context completeness against token costs. Summarization and selective context passing become more important as conversations grow longer.
...
Was this page helpful?YesNo[SubagentsPrevious](/oss/python/langchain/multi-agent/subagents)[SkillsNext](/oss/python/langchain/multi-agent/skills)⌘I[Docs by LangChain home page
