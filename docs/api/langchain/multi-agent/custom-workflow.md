...
Use this file to discover all available pages before exploring further.In the **custom workflow** architecture, you define your own bespoke execution flow using [LangGraph](/oss/python/langgraph/overview). You have complete control over the graph structure—including sequential steps, conditional branches, loops, and parallel execution.

## [​](#key-characteristics)Key characteristics

{' ' * (self.list_depth - 1)}- Complete control over graph structure

{' ' * (self.list_depth - 1)}- Mix deterministic logic with agentic behavior

{' ' * (self.list_depth - 1)}- Support for sequential steps, conditional branches, loops, and parallel execution

{' ' * (self.list_depth - 1)}- Embed other patterns as nodes in your workflow
...
{' ' * (self.list_depth - 1)}- **Model node** (Rewrite): Rewrites the user query for better retrieval using [structured output](/oss/python/langchain/structured-output).

{' ' * (self.list_depth - 1)}- **Deterministic node** (Retrieve): Performs vector similarity search — no LLM involved.

{' ' * (self.list_depth - 1)}- **Agent node** (Agent): Reasons over retrieved context and can fetch additional information via tools.
...
[Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.[Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss/langchain/multi-agent/custom-workflow.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).

Was this page helpful?YesNo[RouterPrevious](/oss/python/langchain/multi-agent/router)[RetrievalNext](/oss/python/langchain/retrieval)⌘I[Docs by LangChain home page
