...
### [​](#what-are-voice-agents)What are voice agents?
...
{' ' * (self.list_depth - 1)}- Customer support

{' ' * (self.list_depth - 1)}- Personal assistants

{' ' * (self.list_depth - 1)}- Hands-free interfaces

{' ' * (self.list_depth - 1)}- Coaching and training

### [​](#how-do-voice-agents-work)How do voice agents work?

At a high level, every voice agent needs to handle three tasks:

{' ' * (self.list_depth - 1)}- **Listen** - capture audio and transcribe it

{' ' * (self.list_depth - 1)}- **Think** - interpret intent, reason, plan

{' ' * (self.list_depth - 1)}- **Speak** - generate audio and stream it back to the user
...
#### [​](#1-stt->-agent->-tts-architecture-the-“sandwich”)1. STT > Agent > TTS architecture (The “Sandwich”)
...
{' ' * (self.list_depth - 1)}- Full control over each component (swap STT/TTS providers as needed)

{' ' * (self.list_depth - 1)}- Access to latest capabilities from modern text-modality models

{' ' * (self.list_depth - 1)}- Transparent behavior with clear boundaries between components

**Cons:**

{' ' * (self.list_depth - 1)}- Requires orchestrating multiple services

{' ' * (self.list_depth - 1)}- Additional complexity in managing the pipeline

{' ' * (self.list_depth - 1)}- Conversion from speech to text loses information (e.g., tone, emotion)

#### [​](#2-speech-to-speech-architecture-s2s)2. Speech-to-Speech architecture (S2S)

Speech-to-speech uses a multimodal model that processes audio input and generates audio output natively.

**Pros:**

{' ' * (self.list_depth - 1)}- Simpler architecture with fewer moving parts

{' ' * (self.list_depth - 1)}- Typically lower latency for simple interactions

{' ' * (self.list_depth - 1)}- Direct audio processing captures tone and other nuances of speech

**Cons:**

{' ' * (self.list_depth - 1)}- Limited model options, greater risk of provider lock-in

{' ' * (self.list_depth - 1)}- Features may lag behind text-modality models

{' ' * (self.list_depth - 1)}- Less transparency in how audio is processed

{' ' * (self.list_depth - 1)}- Reduced controllability and customization options
...
{' ' * (self.list_depth - 1)}- Captures microphone audio and encodes it as PCM

{' ' * (self.list_depth - 1)}- Establishes WebSocket connection to the backend server

{' ' * (self.list_depth - 1)}- Streams audio chunks to the server in real-time

{' ' * (self.list_depth - 1)}- Receives and plays back synthesized speech audio
...
{' ' * (self.list_depth - 1)}- [Speech-to-text (STT)](#1-speech-to-text): Forwards audio to the STT provider (e.g., AssemblyAI), receives transcript events

{' ' * (self.list_depth - 1)}- [Agent](#2-langchain-agent): Processes transcripts with LangChain agent, streams response tokens

{' ' * (self.list_depth - 1)}- [Text-to-speech (TTS)](#3-text-to-speech): Sends agent responses to the TTS provider (e.g., Cartesia), receives audio chunks
...
{' ' * (self.list_depth - 1)}- `stt_chunk`: Partial transcripts provided as the STT service processes audio

{' ' * (self.list_depth - 1)}- `stt_output`: Final, formatted transcripts that trigger agent processing
...
## [​](#2-langchain-agent)2. LangChain agent
...
{' ' * (self.list_depth - 1)}- **Upstream processing**: Passes through all events and sends agent text chunks to the TTS provider

{' ' * (self.list_depth - 1)}- **Audio reception**: Receives synthesized audio chunks from the TTS provider
...
[Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.[Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss/langchain/voice-agent.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).

Was this page helpful?YesNo[Build a SQL agentPrevious](/oss/python/langchain/sql-agent)[Build a personal assistant with subagentsNext](/oss/python/langchain/multi-agent/subagents-personal-assistant)⌘I[Docs by LangChain home page
