...
[Core capabilities](/langsmith/core-capabilities)

# A2A endpoint in Agent Server
Copy page

Use the A2A protocol to enable agent-to-agent communication with distributed tracing in LangSmith.Copy page
> 
...
Use this file to discover all available pages before exploring further.[Agent2Agent (A2A)](https://a2a-protocol.org/latest/) is Google’s protocol for enabling communication between conversational AI agents. [LangSmith implements A2A support](https://docs.langchain.com/langsmith/server-api-ref#tag/a2a/post/a2a/%7Bassistant_id%7D), allowing your agents to communicate with other A2A-compatible agents through a standardized protocol.
...
{' ' * (self.list_depth - 1)}- **message/send**: Send a message to an assistant and receive a complete response

{' ' * (self.list_depth - 1)}- **message/stream**: Send a message and stream responses in real-time using Server-Sent Events (SSE)

{' ' * (self.list_depth - 1)}- **tasks/get**: Retrieve the status and results of a previously created task
...
{' ' * (self.list_depth - 1)}- `langgraph-api >= 0.4.21`
...
{' ' * (self.list_depth - 1)}- Upgrade to use langgraph-api>=0.4.21.

{' ' * (self.list_depth - 1)}- Deploy your agent with message-based state structure.

{' ' * (self.list_depth - 1)}- Connect with other A2A-compatible agents using the endpoint.

## [​](#creating-an-a2a-compatible-agent)Creating an A2A-compatible agent
...
{' ' * (self.list_depth - 1)}- `contextId`: Groups messages into a conversation thread (like a session ID)

{' ' * (self.list_depth - 1)}- `taskId`: Identifies each individual request within that conversation
...
{' ' * (self.list_depth - 1)}- [Two LangGraph agents communicating](https://github.com/langchain-samples/A2A-langgraph) - Example of two LangGraph agents using the A2A protocol

{' ' * (self.list_depth - 1)}- [Google ADK agent with LangChain agent](https://github.com/langchain-samples/A2A-google-adk) - Example of a Google ADK agent interacting with a LangChain agent using the A2A protocol
...
### [​](#how-contextid-maps-to-thread_id)How contextId maps to thread_id
...
{' ' * (self.list_depth - 1)}- On the first message, the client omits `contextId`. The server generates one and returns it in the response.

{' ' * (self.list_depth - 1)}- The client passes the `contextId` in all subsequent messages to maintain conversation continuity.

{' ' * (self.list_depth - 1)}- Agent Server maps the `contextId` to `thread_id` in LangSmith [metadata](/langsmith/add-metadata-tags), so all turns appear in the same thread.

### [​](#tracing-across-multiple-agents)Tracing across multiple agents
...
### [​](#receive-thread_id-in-non-langgraph-agents)Receive thread_id in non-LangGraph agents
...
### [​](#view-traces-in-langsmith)View traces in LangSmith
...
[Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.[Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/langsmith/server-a2a.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).

Was this page helpful?YesNo[MCP endpoint in Agent ServerPrevious](/langsmith/server-mcp)[Distributed tracing with Agent ServerNext](/langsmith/agent-server-distributed-tracing)⌘I[Docs by LangChain home page
