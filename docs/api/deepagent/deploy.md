...
[Deployment guides](/langsmith/deploy-managed-deep-agent)

# Managed Deep Agents
Copy page

Create, run, and operate Managed Deep Agents with the private preview API.Copy page
> 
...
Use this file to discover all available pages before exploring further.Managed Deep Agents is an API-first hosted runtime for creating, running, and operating deep agents in LangSmith. This guide walks through the full flow: define the agent, configure tools, create the managed agent, create a thread, stream a run, and inspect the result in LangSmith.
...
## [​](#when-to-use-managed-deep-agents)When to use Managed Deep Agents

Use Managed Deep Agents when you want to:

{' ' * (self.list_depth - 1)}- Create and manage deep agents programmatically.

{' ' * (self.list_depth - 1)}- Run long-running agents without standing up a custom agent server.

{' ' * (self.list_depth - 1)}- Stream runs and preserve durable thread state.

{' ' * (self.list_depth - 1)}- Use a managed file tree (instructions, skills, subagents, tools, and a `/memories/` workspace) without running your own storage.

{' ' * (self.list_depth - 1)}- Inspect traces and agent behavior in LangSmith.
...
{' ' * (self.list_depth - 1)}- Managed Deep Agents private preview access.

{' ' * (self.list_depth - 1)}- A LangSmith API key for a workspace with private preview access.

{' ' * (self.list_depth - 1)}- A Deep Agent definition, including instructions and any tool configuration you want the managed runtime to use.
...
{' ' * (self.list_depth - 1)}- Python (httpx)
{' ' * (self.list_depth - 1)}- JavaScript (fetch)
```
pip install httpx
...
### [​](#list-and-inspect-mcp-servers)List and inspect MCP servers
...
## [​](#create-a-managed-agent)Create a managed agent
...
## [​](#manage-existing-agents)Manage existing agents
...
## [​](#inspect-the-result-in-langsmith)Inspect the result in LangSmith

Managed Deep Agents runs are traced in LangSmith, so teams can debug behavior and inspect tool calls. Use traces to review:

{' ' * (self.list_depth - 1)}- The user’s input and the agent’s final response.

{' ' * (self.list_depth - 1)}- Model calls and tool calls.

{' ' * (self.list_depth - 1)}- Subagent activity.

{' ' * (self.list_depth - 1)}- Files and runtime state created during the run.
...
### [​](#thread-retention)Thread retention
...
## [​](#built-on-open-source-deep-agents)Built on open-source Deep Agents
...
## [​](#private-preview-scope)Private preview scope
...
[Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.[Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/langsmith/deploy-managed-deep-agent.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).

Was this page helpful?YesNo[Implement a CI/CD pipeline using LangSmith Deployment and EvaluationPrevious](/langsmith/cicd-pipeline-example)[Deploy on CloudNext](/langsmith/deploy-to-cloud)⌘I[Docs by LangChain home page
