...
Package per-provider and per-model defaults that Deep Agents applies when a model is selectedCopy page
> 
...
[​](#param-base-system-prompt)base_system_promptstringReplace the base Deep Agents system prompt (`CUSTOM` in [Prompt assembly](/oss/python/deepagents/customization#prompt-assembly)).
[​](#param-system-prompt-suffix)system_prompt_suffixstringAppend text to the assembled base prompt (`SUFFIX` in [Prompt assembly](/oss/python/deepagents/customization#prompt-assembly)); applied to the main agent, declarative subagents, and the auto-added general-purpose subagent.
[​](#param-tool-description-overrides)tool_description_overridesMapping[str, str]Override individual tool descriptions, keyed by tool name.
[​](#param-excluded-tools)excluded_toolsfrozenset[str]Remove specific harness-level tools from the tool set. Matched by tool name (string), applied as a post-injection filter so it can drop both user-supplied tools and tools added by harness middleware. See [Running without the default filesystem tools](/oss/python/deepagents/harness#virtual-filesystem-access) for a worked example.
[​](#param-excluded-middleware)excluded_middlewarefrozenset[type[AgentMiddleware] | str]Strip specific middleware classes from the stack. Accepts middleware classes or string names.
[​](#param-extra-middleware)extra_middlewareSequence[AgentMiddleware] | Callable[[], Sequence[AgentMiddleware]]Append middleware to every stack this profile applies to.
[​](#param-general-purpose-subagent)general_purpose_subagentGeneralPurposeSubagentProfileDisable, rename, or re-prompt the general-purpose subagent. When this field’s `system_prompt` is set alongside `base_system_prompt`, the general-purpose-specific subagent prompt wins—see [General-purpose subagent prompt](/oss/python/deepagents/customization#general-purpose-subagent-prompt).
...
{' ' * (self.list_depth - 1)}- A middleware *class* (matched by exact type), or a plain string that matches `AgentMiddleware.name`. Use plain strings for built-ins and public aliases such as `"SummarizationMiddleware"`.

{' ' * (self.list_depth - 1)}- An `module:Class` import ref (for example, `"my_pkg.middleware:TelemetryMiddleware"`) to target an exact middleware class from a config file. Import refs resolve lazily, so use them only for trusted local configuration — loading one imports Python code.

Lookup order for preconfigured model instancesWhen you pass a preconfigured chat model instance instead of a `provider:model` string, the harness synthesizes the canonical `provider:identifier` key from the instance and looks it up in this order:

{' ' * (self.list_depth - 1)}- Exact `provider:identifier` match

{' ' * (self.list_depth - 1)}- Identifier-only (only when the identifier already contains `:`)

{' ' * (self.list_depth - 1)}- Provider-only fallback

## [​](#registration-keys)Registration keys

Both profile types use the same key format:

{' ' * (self.list_depth - 1)}- **Provider-level** — a bare provider name like `"openai"` applies to every model from that provider.

{' ' * (self.list_depth - 1)}- **Model-level** — a fully qualified `provider:model` key like `"openai:gpt-5.4"` applies only to that specific model.
...
[​](#param-init-kwargs)init_kwargsMapping[str, Any]Static initialization arguments forwarded to `init_chat_model`.
[​](#param-pre-init)pre_initCallable[[str], None]Side effects to run before construction (for example, credential validation).
[​](#param-init-kwargs-factory)init_kwargs_factoryCallable[[], dict[str, Any]]Kwargs derived from runtime state (for example, headers pulled from environment variables).
...
{' ' * (self.list_depth - 1)}- Class-form `excluded_middleware` entries serialize as a public alias (when the class exposes one via `serialized_name: ClassVar[str]`) or as a `module:Class` import ref.

{' ' * (self.list_depth - 1)}- Non-empty `extra_middleware` and middleware classes declared in `__main__` or inside a function scope cannot be serialized — export raises `ValueError`.
...
{' ' * (self.list_depth - 1)}- [Harness](/oss/python/deepagents/harness) — overview of harness capabilities

{' ' * (self.list_depth - 1)}- [Models](/oss/python/deepagents/models) — configure model providers and parameters

{' ' * (self.list_depth - 1)}- [Customization](/oss/python/deepagents/customization) — full `create_deep_agent` configuration surface

---

[Connect these docs](/use-these-docs) to Claude, VSCode, and more via MCP for real-time answers.[Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss/deepagents/profiles.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).

Was this page helpful?YesNo[InterpretersPrevious](/oss/python/deepagents/interpreters)[Event streamingNext](/oss/python/deepagents/event-streaming)⌘I[Docs by LangChain home page
